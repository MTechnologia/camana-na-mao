...
Correções humanas (admin, automacao) e, no futuro, do cidadão alimentam a tabela `report_classification_feedback`. O orquestrador **reutiliza** essas correções antes da heurística/LLM para descrições **parecidas**.

## Componentes

| Peça | Função |
|------|--------|
| `public.report_classification_feedback` | Armazena `report_type`, `original_*`, `corrected_*`, `description_excerpt`, `source` (`admin` \| `automacao` \| `user`). |
...
| `automacao-callback` | Quando a categoria validada difere da salva, insere com `source: 'automacao'`. |
