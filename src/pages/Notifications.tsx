...
const Notifications = () => {
  ...
  const subscriptionNotifications = notifications.filter((n) =>
    SUBSCRIPTION_NOTIFICATION_TYPES.has(n.type),
  );
  ...
  const resolveNotificationActionUrl = (notification: (typeof notifications)[0]) => {
    ...
    const reportId = typeof metadata.report_id === "string" ? metadata.report_id : undefined;
    const patternId = typeof metadata.pattern_id === "string" ? metadata.pattern_id : undefined;
    const lineId = typeof metadata.line_id === "string" ? metadata.line_id : undefined;
  ...
  return (
    <div className="min-h-screen bg-background pb-24 pt-[60px]">
      ...
      <div className="p-4 space-y-4">
        ...
        <Tabs
          value={filter}
          onValueChange={(v) => setFilter(v as "all" | "unread" | "subscriptions")}
        >
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="all">Todas ({notifications.length})</TabsTrigger>
            <TabsTrigger value="unread">Não Lidas ({unreadCount})</TabsTrigger>
            <TabsTrigger value="subscriptions">
              Acompanhamento ({subscriptionNotifications.length})
            </TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        {filteredNotifications.length === 0 ? (
          ...
          <div className="space-y-2">
            {filteredNotifications.map((notification) => {
              const isSubscriptionNotification = SUBSCRIPTION_NOTIFICATION_TYPES.has(
                notification.type,
              );
              return (
                <div
