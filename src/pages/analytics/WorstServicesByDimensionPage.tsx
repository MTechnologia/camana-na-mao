...
export default function WorstServicesByDimensionPage() {
  ...
  const { rows, loading, error, refetch } = useWorstServicesByDimension(dimension, 25);
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.service_id}>
                    ...
                    <TableCell className="text-right tabular-nums">{Number(r.avg_score).toFixed(2)}</TableCell>
