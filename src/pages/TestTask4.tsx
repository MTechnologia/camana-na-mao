          ...
          <CardContent className="p-6 space-y-6">
            <div className="bg-muted p-4 rounded-lg relative">
              <p className="text-sm font-medium mb-4">
                Você tem alguma **sugestão de melhoria** ou quer deixar um comentário extra? (Digite
                abaixo ou diga "pular")
              </p>
            ...
            {result && (
              <Card
                className={`border ${result.bypassed ? "bg-orange-500/10 border-orange-500/20" : result.status.includes("Erro") ? "bg-red-500/10 border-red-500/20" : "bg-green-500/10 border-green-500/20"}`}
              >
                <CardContent className="p-4">
                  <h3 className="font-semibold mb-1">Resultado do Processamento (Backend):</h3>
                  <div className="space-y-1 text-sm">
                    <p>
                      <strong>Status de Validação:</strong> {result.status}
                    </p>
                    <p>
                      <strong>Valor salvo no Banco:</strong> <code>{result.finalValue}</code>
                    </p>
                    {result.bypassed && (
