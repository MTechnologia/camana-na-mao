...
const typeConfig: Record<ManifestType, { label: string; shortLabel: string; icon: typeof Building2; color: string }> = {
  urban: { label: 'Urbana', shortLabel: 'Urb', icon: Building2, color: 'bg-blue-500/10 text-blue-600 border-blue-500/20' },
  transport: { label: 'Transporte', shortLabel: 'Transp', icon: Bus, color: 'bg-purple-500/10 text-purple-600 border-purple-500/20' },
  evaluation: { label: 'Avaliação', shortLabel: 'Aval', icon: Star, color: 'bg-amber-500/10 text-amber-600 border-amber-500/20' },
  feedback: { label: 'Feedback', shortLabel: 'Feed', icon: MessageSquare, color: 'bg-cyan-500/10 text-cyan-600 border-cyan-500/20' },
};

const statusConfig: Record<string, { label: string; shortLabel: string; color: string; icon: typeof Clock }> = {
  pending: { label: 'Pendente', shortLabel: 'Pend.', color: 'bg-yellow-500/10 text-yellow-600 border-yellow-500/20', icon: Clock },
  in_progress: { label: 'Em Andamento', shortLabel: 'Andam.', color: 'bg-blue-500/10 text-blue-600 border-blue-500/20', icon: TrendingUp },
  resolved: { label: 'Resolvido', shortLabel: 'Resolv.', color: 'bg-green-500/10 text-green-600 border-green-500/20', icon: CheckCircle2 },
  rejected: { label: 'Rejeitado', shortLabel: 'Rejeit.', color: 'bg-red-500/10 text-red-600 border-red-500/20', icon: XCircle },
  completed: { label: 'Concluída', shortLabel: 'Concl.', color: 'bg-green-500/10 text-green-600 border-green-500/20', icon: CheckCircle2 },
};
      ...
      <div className="hidden sm:flex lg:hidden items-center gap-2 p-3">
        ...
        >
          <div className="flex items-center gap-1.5 mb-0.5">
            <Badge variant="outline" className={`${typeConfig[manifest.type].color} text-[10px] px-1.5 py-0 shrink-0`}>
              <TypeIcon className="h-2.5 w-2.5 mr-0.5" />
              {typeConfig[manifest.type].shortLabel}
            </Badge>
            {manifest.severity && severityConfig[manifest.severity] && (
              <Badge variant="outline" className={`${severityConfig[manifest.severity].color} text-[10px] px-1.5 py-0 shrink-0`}>
                {severityConfig[manifest.severity].label}
              </Badge>
            )}
            {manifest.urban_data?.risk_level === 'critical' && (
        ...
        >
          <SelectTrigger 
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <span className="truncate">{statusConfig[manifest.status]?.shortLabel || manifest.status}</span>
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="pending">
              <span className="flex items-center gap-1">
