...
import {
  ...
  isUrbanConfirmCorrectQuickReply,
} from "@/lib/parseUrbanReportPreview";
...
import {
  SimilarUrbanReportsInChat,
  parseSimilarUrbanReportsB64,
} from "./SimilarUrbanReportsInChat";
import {
...
import NearbyServicesFiltersInline, {
  type NearbyFiltersValues,
} from "./NearbyServicesFiltersInline";
...
interface ChatMessageBubbleProps {
  ...
  onRequestAudienciasWithFilters?: (filters: {
    tema: string;
    regiao: string;
    dateFrom: string;
    dateTo: string;
...
const ChatMessageBubble = ({
  ...
  const [ratingCommentDisplayOverride, setRatingCommentDisplayOverride] = useState<string | null>(
    null,
  );
  ...
  const subcategoryPickerMatch = !isUser
    ? message.content.match(/\[SUBCATEGORY_PICKER:([a-z_]+)\]/i)
    : null;
  ...
  const hasRecurrenceFrequencyPicker =
    !isUser && message.content.includes("[RECURRENCE_FREQUENCY_PICKER]");
  const hasImpactPicker = !isUser && message.content.includes("[IMPACT_PICKER]");
  const hasAccessibilityChecklist =
    !isUser && message.content.includes("[ACCESSIBILITY_CHECKLIST]");
  ...
  const shouldShowAudienciasCta = useMemo(() => {
    ...
    const isListingAudiencias =
      ...
      (content.includes("inscrever-se") &&
        (content.includes("agendadas") || content.includes("próximas")));
    ...
    queryFn: async () => {
      ...
      while (true) {
        const { data, error } = await supabase
          .from("audiencias")
          .select(
            "id, titulo, descricao, data, hora, local, tema, status, comissao, convidados, projeto_referencia, link_transmissao, mais_informacoes, inscricoes_abertas, vagas_disponiveis",
          )
  ...
  const audienciasFiltradasNoChat = useMemo(() => {
    ...
    const filtered = audienciasData.filter((a) => {
      ...
      const matchesDate =
        ...
        (audienciaChatDateFrom &&
          audienciaChatDateTo &&
          itemDate >= audienciaChatDateFrom &&
          itemDate <= audienciaChatDateTo) ||
  ...
  const treatCepQuestionAsUrbanLocationStep = useMemo(() => {
    if (isUser || !isLastAssistantMessage || addressSelected || locationMethodSelected)
      return false;
    ...
    if (
      /\[\s*LOCATION_METHOD_PICKER\s*\]/i.test(raw) ||
      raw.includes("[FIELD_REQUEST:location_method]")
    ) {
      return false;
    }
  ...
  }, [isUser, message.content, impactSelected, hasImpactPicker, isLastAssistantMessage]);
  ...
  const assistantRawForDisplay = useMemo(() => {
    if (!ratingCommentDisplayOverride || !message.content.includes("[RATING_CREATED:")) {
  ...
  const withStepLineBreaks = (text: string): string => {
    if (text.includes("**Passo a passo:**") && text.includes("•"))
      return text.replace(/\n/g, "  \n");
    return text;
  };
  ...
  const urbanReportPreviewParsed = useMemo(
    () => parseUrbanReportPreview(cleanContent),
    [cleanContent],
  );
  ...
  return (
    <div className={cn("flex gap-3 items-start", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      <div className={cn("flex flex-col gap-1 max-w-[75%]", isUser ? "items-end" : "items-start")}>
          ...
          {isUser ? (
            ...
            <div className="w-full">
                  ...
                  >
                    <ReactMarkdown
                      components={{
                        ...
                        ol: ({ children }) => (
                          <ol className="text-sm list-decimal pl-4 mb-2">{children}</ol>
                        ),
                ...
                <div className="mt-3 pt-3 border-t border-border/60 space-y-2">
                  {!ratingCommentEditing ? (
                    ...
                    <div className="space-y-2">
                      <label
                        htmlFor={`rating-comment-edit-${message.id}`}
                              ...
                              <div className="space-y-2 text-muted-foreground pl-6">
                                ...
                                {a.mais_informacoes?.trim() && (
                                  <div className="space-y-0.5">
                                    ...
                                    {(() => {
                                      const email = extrairEmailDeMaisInformacoes(
                                        a.mais_informacoes,
                                      );
                                      if (email) {
                                        return (
                                      ...
                                      return (
                                        <span className="text-xs">
                                          {a.mais_informacoes
                                            .replace(/^Mais\s+informa[cç][oõ]es\s*:\s*/i, "")
                                            .trim()}
                                        </span>
                                      );
                                    })()}
              ...
              {!isUser && similarTransportReportsPayload && (
                <SimilarTransportReportsInChat
                  payload={similarTransportReportsPayload}
                  className="mt-3"
                />
              )}
            </div>
        ...
        {(hasDirectionPicker || isAskingForDirection) &&
          !directionSelected &&
          isLastAssistantMessage && <InlineDirectionPicker onSelect={handleDirectionSelected} />}
        ...
        {(hasImpactPicker || isAskingForPersonalImpact) &&
          !impactSelected &&
          isLastAssistantMessage &&
          onImpactSelected && <InlineImpactPicker onSelect={handleImpactSelected} />}
        ...
        {(hasRatingPicker || isAskingForRating) &&
          ...
          !ratingSelected &&
          isLastAssistantMessage &&
          onRatingSelected && <InlineRatingPicker onSelect={handleRatingSelected} />}
        ...
        {(hasWaitTimePicker || isAskingForWaitTime) &&
          ...
          !waitTimeSelected &&
          isLastAssistantMessage &&
          onWaitTimeSelected && <WaitTimePicker onSelect={handleWaitTimeSelected} />}
        ...
        {(hasServiceTypePicker || isAskingForServiceType) &&
          !serviceTypeSelected &&
          isLastAssistantMessage && (
          ...
          <div className="mt-3 flex flex-col gap-2 w-full max-w-[280px]">
            ...
            >
              <span className="truncate flex-1 text-left">
                Sim, iniciar{" "}
                {JOURNEY_NAMES[journeySwitchMatch.newJourney] || journeySwitchMatch.newJourney}
              </span>
              <ArrowRight className="h-4 w-4 ml-2 flex-shrink-0" />
            </Button>
            <Button
            ...
            >
              <span className="truncate flex-1 text-left">
                Não, continuar{" "}
                {JOURNEY_NAMES[journeySwitchMatch.currentJourney] ||
                  journeySwitchMatch.currentJourney}
              </span>
          ...
          <div className="mt-3 flex flex-col gap-3 w-full max-w-[320px]">
            <div className="flex flex-col gap-2">
              {hasInscricoesAbertas ? (
                ...
                >
                  {showAudienciaInscricaoInline
                    ? "Ocultar formulário"
                    : "Inscrever-se aqui no chat"}
                </Button>
              ) : (
                <Button
                  disabled
                  className="w-full bg-muted text-muted-foreground cursor-default min-h-[40px]"
                >
              ...
              <>
                <div className="space-y-2 rounded-lg border border-border/60 bg-muted/30 p-3">
                  <p className="text-sm font-semibold text-foreground">
                    Listagem de audiências públicas
                  </p>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setAudienciaChatTema(v === AUDIENCIA_CHAT_ANY ? "" : v)
                        }
                      >
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setAudienciaChatRegiao(v === AUDIENCIA_CHAT_ANY ? "" : v)
                        }
                      >
                        <SelectTrigger className="h-8 text-xs">
