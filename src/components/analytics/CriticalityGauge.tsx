...
export const CriticalityGauge = ({
  ...
  return (
    <div
      className={cn("flex flex-col items-center justify-center", onClick && "cursor-pointer group")}
      onClick={onClick}
    >
      <div
        className={cn(
      ...
      >
        ...
        >
          {score >= 60 && <AlertTriangle className="h-6 w-6 text-chart-5 mb-1" />}
          <p className={`${currentSize.text} font-bold`} style={{ color }}>
            {score}%
          </p>
          <p className={`${currentSize.subtext} text-muted-foreground`}>{level}</p>
        </motion.div>
      </div>
