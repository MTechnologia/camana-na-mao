...
export function QuickFilterPills({
  ...
  const allOptions = showAllOption ? [{ value: allValue, label: allLabel }, ...options] : options;
      ...
      <div className="flex gap-2 pb-2">
        {allOptions.map((option) => {
          ...
          return (
            ...
            >
              {typeof option.icon === "string" && <span className="mr-1">{option.icon}</span>}
              ...
              {option.count !== undefined && (
                <span
                  className={cn("ml-1.5 text-xs", active ? "opacity-80" : "text-muted-foreground")}
                >
                  {option.count}
                </span>
              )}
