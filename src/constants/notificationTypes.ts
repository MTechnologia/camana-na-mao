...
export const getNotificationPriority = (priority: string) => {
  return (
    NOTIFICATION_PRIORITIES[priority as keyof typeof NOTIFICATION_PRIORITIES] ||
    NOTIFICATION_PRIORITIES.normal
  );
};

// Options for filter selects
export const NOTIFICATION_TYPE_OPTIONS = Object.entries(NOTIFICATION_TYPES).map(
  ([value, config]) => ({
    value,
    label: config.label,
  }),
...
export const NOTIFICATION_PRIORITY_OPTIONS = Object.entries(NOTIFICATION_PRIORITIES).map(
  ([value, config]) => ({
    value,
    label: config.label,
  }),
