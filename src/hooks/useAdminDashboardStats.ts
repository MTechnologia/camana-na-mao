  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const dashboardsTrend = totalDashboards ? Math.round((monthDashboards || 0) / (totalDashboards / 100)) : 0;
        const exportsTrend = totalExports ? Math.round((weekExports || 0) / (totalExports / 100)) : 0;
