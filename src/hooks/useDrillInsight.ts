...
import { citizenReportStatusLabel, normalizeCitizenReportStatus } from "@/lib/citizenReportStatus";
...
export const useDrillInsight = (baseFilters: ReportsAnalyticsFilters = {}) => {
  ...
  const generateInsight = (context: DrillContext, stats: DrillStats): string => {
    ...
    const trendText =
      stats.trend > 0
        ? `aumento de ${stats.trend}%`
        : stats.trend < 0
          ? `redução de ${Math.abs(stats.trend)}%`
          : "estável";
    ...
    switch (type) {
      ...
      case "sentiment": {
        const sentimentLabel =
          value === "positive" ? "positivos" : value === "negative" ? "negativos" : "neutros";
  ...
  const mapServiceReports = (data: Record<string, unknown>[]): DrillReport[] => {
    return data.map((r) => {
      ...
      return {
        ...
        status: "Avaliação",
        severity: "—",
        location_address: drillStrNullable(svcObj?.address) || drillStrNullable(svcObj?.district),
        created_at: drillStr(r.created_at),
        user_id: drillStr(r.user_id),
  ...
  const searchByKeyword = useCallback(async (keyword: string) => {
    ...
    try {
      const { data: urbanData, error: urbanError } = await supabase
        ...
        .select("*")
        .or(
          `description.ilike.%${keyword}%,category.ilike.%${keyword}%,location_address.ilike.%${keyword}%`,
        );
      ...
      const { data: transportData, error: transportError } = await supabase
        ...
        .select("*")
        .or(
          `description.ilike.%${keyword}%,impact_description.ilike.%${keyword}%,location.ilike.%${keyword}%`,
        );
    ...
    try {
      ...
      const filteredUrban = (urbanData || []).filter((r: Record<string, unknown>) => {
        const reportSentiment = (
          r.ai_classification as { sentiment?: string } | undefined
      ...
      const context: DrillContext = {
        type: "sentiment",
        value: sentiment,
        label: `Sentimento: ${sentimentLabel}`,
      };
      const insight = generateInsight(context, stats);
  ...
  const searchBySeverity = useCallback(async (severity: string) => {
    ...
    try {
      ...
      const displayLabel = canonicalSeverity ? SEVERITY_LABELS[canonicalSeverity] : severity;
  ...
  const searchByGender = useCallback(
    async (gender: string) => {
      setState((prev) => ({ ...prev, isLoading: true }));
    ...
    async (ageGroup: string) => {
      ...
      try {
        ...
        const context: DrillContext = {
          type: "age",
          value: ageKey,
          label: `Faixa Etária: ${displayLabel}`,
        };
        applyDemographicDrillResult(
          allReports,
          evaluationCount,
  ...
  const searchByRace = useCallback(
    async (race: string) => {
      setState((prev) => ({ ...prev, isLoading: true }));
      ...
      try {
        ...
        const { urban, transport, evaluationCount } = await fetchDrillReportsByRace(
          race,
          drillFilters,
        );
        const allReports = [...mapUrbanReports(urban), ...mapTransportReports(transport)];
  ...
  const searchBySocialClass = useCallback(
    async (socialClass: string) => {
      setState((prev) => ({ ...prev, isLoading: true }));
   ...
   */
  const searchByCrossDemographic = useCallback(
    async (
      ...
      try {
        ...
        let allReports: DrillReport[] = [];
        if (category === "Urbano") {
          const { data } = await supabase.from("urban_reports").select("*").in("user_id", userIds);
          allReports = mapUrbanReports(data || []);
   ...
   */
  const searchByCrossCellReports = useCallback(
    async (
      rowLabel: string,
      colLabel: string,
      ...
      try {
        ...
        const transportIds = cellReports
          .filter((r) => r.category === "Transporte")
          .map((r) => r.id);
        const evaluationIds = cellReports
          .filter((r) => r.category === "Avaliação")
          .map((r) => r.id);
