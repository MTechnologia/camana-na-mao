  ...
  const fetchAnalytics = useCallback(
    async (isMounted: boolean = true) => {
      try {
        ...
        const mappedGender =
          filters.gender === "Não informado" ? "not_informed" : filters.gender || null;

        const mappedRace = filters.race === "Não informado" ? "not_informed" : filters.race || null;

        const mappedSocialClass =
          filters.socialClass === "Não informado" ? "not_informed" : filters.socialClass || null;
        ...
        let categories = (demographicsFromRpc?.category_distribution || []).map(
        ...
        // Processar regiões
        const byRegion = (demographicsFromRpc?.neighborhood_distribution || []).map(
          (n: { neighborhood?: string; count?: number }) => ({
        ...
        const statusMap = new Map<string, number>(
          statusData.map((s: { status?: string; count?: number }) => [
            s.status ?? "",
            Number(s.count) || 0,
          ]),
        );
        ...
        let byStatus = [
          ...
          {
            status: "Em Análise",
            count: statusMap.get("in_progress") || 0,
        ...
        try {
          ...
          const includeUrban = categorySlice.isAll || categorySlice.urbanCategories.length > 0;
          const includeTransport =
            !urbanOnly && (categorySlice.isAll || categorySlice.transportSubcategories.length > 0);
          const includeEvaluations =
            !urbanOnly && (categorySlice.isAll || categorySlice.publicServiceTypes.length > 0);
          ...
          responseTime = buildResponseTimeDrillStats(filterResponseTimeRecords(rtRecords, filters));
          ...
          const categoriesFromTerritory =
            buildCategoryDistributionFromTerritoryRows(territoryFiltered);
        ...
        const totalLikes = urbanForTimeline.reduce(
          (sum, r: Record<string, unknown>) =>
            sum + ((r.urban_report_likes as { count?: number }[])?.[0]?.count || 0),
          0,
        );
        const totalComments = urbanForTimeline.reduce(
          (sum, r: Record<string, unknown>) => sum + (r.urban_report_comments?.[0]?.count || 0),
          0,
        );
        ...
        const withInteraction = urbanForTimeline.filter(
          (r: Record<string, unknown>) =>
            (r.urban_report_likes?.[0]?.count || 0) > 0 ||
            (r.urban_report_comments?.[0]?.count || 0) > 0,
        ).length;
        const withLikes = urbanForTimeline.filter(
          (r: Record<string, unknown>) => (r.urban_report_likes?.[0]?.count || 0) > 0,
        ).length;
        ...
        const conversionFunnel: FunnelStep[] = [
          {
            ...
            count: engagementBaseTotal,
            percentage: engagementBaseTotal > 0 ? 100 : 0,
            color: "hsl(var(--chart-2))",
          },
          {
            label: "Com Interação",
            count: withInteraction,
            percentage: engagementBaseTotal > 0 ? (withInteraction / engagementBaseTotal) * 100 : 0,
            color: "hsl(var(--chart-6))",
          },
          {
            label: "Apoiados",
            count: withLikes,
            percentage: engagementBaseTotal > 0 ? (withLikes / engagementBaseTotal) * 100 : 0,
            color: "hsl(var(--chart-3))",
          },
          {
            label: "Resolvidos",
            count: resolvedUrban,
            percentage: engagementBaseTotal > 0 ? (resolvedUrban / engagementBaseTotal) * 100 : 0,
            color: "hsl(var(--chart-1))",
          },
        ];
        ...
        if (isMounted) {
          setStats({
            ...
            engagement: {
              ...
              avgCommentsPerReport:
                engagementBaseTotal > 0 ? totalComments / engagementBaseTotal : 0,
              likesTrend: 0,
              commentsTrend: 0,
              topReports,
              conversionFunnel,
