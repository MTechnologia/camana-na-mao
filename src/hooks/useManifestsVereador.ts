...
export const useManifestsVereador = () => {
  ...
  const fetchManifests = useCallback(async () => {
    ...
    try {
      ...
      const urbanIds = (referrals ?? []).flatMap((item) =>
        item.urban_report_id ? [item.urban_report_id] : [],
      );
      const transportIds = (referrals ?? []).flatMap((item) =>
        item.transport_report_id ? [item.transport_report_id] : [],
      );
      const serviceIds = (referrals ?? []).flatMap((item) =>
      ...
      const [urbanResult, transportResult, serviceResult] = await Promise.all([
        ...
        transportIds.length > 0
          ? supabase
              ...
              .select(
                "id, report_type, sub_category, description, status, protocol_code, created_at",
              )
              .in("id", transportIds)
          : Promise.resolve({ data: [], error: null }),
        serviceIds.length > 0
