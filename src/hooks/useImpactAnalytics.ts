...
export const useImpactAnalytics = (filters: ImpactFilters = {}) => {
  ...
  const fetchImpactAnalytics = useCallback(
    async (isMounted: boolean) => {
      try {
        ...
        let query = supabase
          .from("urban_reports")
          .select(
            "category, risk_level, risk_types, affected_scope, affected_estimate, urgency_reason",
          );
        ...
        const categoryRiskMap = new Map<
          string,
          { critical: number; moderate: number; low: number; none: number }
        >();
        ...
        const byRiskLevel = [
          {
            level: "Crítico",
            count: riskLevelCounts.get("critical") || 0,
            percentage: total > 0 ? ((riskLevelCounts.get("critical") || 0) / total) * 100 : 0,
            color: riskLevelColors.critical,
          },
          {
            level: "Moderado",
            count: riskLevelCounts.get("moderate") || 0,
            percentage: total > 0 ? ((riskLevelCounts.get("moderate") || 0) / total) * 100 : 0,
            color: riskLevelColors.moderate,
          },
          {
            level: "Baixo",
            count: riskLevelCounts.get("low") || 0,
            percentage: total > 0 ? ((riskLevelCounts.get("low") || 0) / total) * 100 : 0,
            color: riskLevelColors.low,
          },
          {
            level: "Nenhum",
            count: riskLevelCounts.get("none") || 0,
            percentage: total > 0 ? ((riskLevelCounts.get("none") || 0) / total) * 100 : 0,
            color: riskLevelColors.none,
          },
        ];
        ...
        const totalWithImpact =
          (riskLevelCounts.get("critical") || 0) +
        ...
        setStats({
          ...
          avgAffectedPerReport:
            reportsWithEstimate > 0 ? totalAffectedEstimate / reportsWithEstimate : 0,
          riskTypesFrequency,
          urgencyKeywords,
          categoryRiskMatrix,
