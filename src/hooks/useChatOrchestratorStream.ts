  ...
  const sendMessage = useCallback(
    async (
      ...
      const hasOptimisticMessage =
        !!lastMessage && lastMessage.role === "user" && lastMessage.content === trimmedContent;

      // Evita segunda requisição idêntica (ex.: duplo disparo do efeito ou duplo clique antes da resposta)
      if (isLoading && lastMessage?.role === "user" && lastMessage.content === trimmedContent) {
        console.warn(
          "[useUnifiedAIChat] Ignoring duplicate send (same text while request in flight)",
        );
        return;
      }
      ...
      try {
        ...
        if (refreshError || !refreshedSession) {
          console.warn(
            "[useUnifiedAIChat] Refresh failed, trying getSession as fallback...",
            refreshError,
          );
          const {
          ...
          if (currentError || !currentSession) {
            console.error(
              "[useUnifiedAIChat] Both refresh and getSession failed:",
        ...
        try {
          ...
          if (tokenParts.length === 3) {
            ...
            if (exp && exp < now + 30) {
              console.warn(
                "[useUnifiedAIChat] Token is expired or expiring soon, attempting another refresh...",
              );
              const {
                data: { session: finalRefreshedSession },
                error: finalRefreshError,
              } = await supabase.auth.refreshSession();

              if (finalRefreshError || !finalRefreshedSession?.access_token) {
                console.error(
                  "[useUnifiedAIChat] Failed to refresh expired token:",
                  finalRefreshError,
                );
                await supabase.auth.signOut();
                toast({
        ...
        const payload = {
          messages: effectiveMessages.map((msg) => ({
            ...
            content: msg.content,
            ...(msg.attachmentUrls &&
              msg.attachmentUrls.length > 0 && { attachmentUrls: msg.attachmentUrls }),
          })),
        ...
        console.log(
          "[useUnifiedAIChat] Payload message count:",
        ...
        if (!supabaseAnonKey) {
          console.warn(
            "[useUnifiedAIChat] CAMARA_PUBLISHABLE_KEY / VITE_SUPABASE_PUBLISHABLE_KEY ausente — chamadas à Edge Function podem falhar.",
          );
        }
        ...
        const processSSELine = (line: string) => {
          ...
          try {
            ...
            if (content) {
              ...
              for (const {
                type: progressType,
                jsonStr: progressJsonStr,
              } of extractCollectionProgressJsonObjects(assistantMessage)) {
                ...
                try {
                  ...
                  if (progressResult.ignoredStale) {
                    console.log(
                      "[useUnifiedAIChat] Ignoring stale COLLECTION_PROGRESS:",
                      type,
                      "- user switched journey",
                    );
                    continue;
                  }
                  ...
                  if (progressResult.fieldsPatch) {
                    ...
                    console.log(
                      "[useUnifiedAIChat] Ignoring non-structured type:",
                      type,
                      "- keeping current journey",
                    );
                  }
                } catch (e) {
                  console.warn(
                    "[useUnifiedAIChat] Failed to parse collection progress:",
                    progressJsonStr,
                    e,
                  );
              ...
              if (transportMatch && !createdReport) {
                ...
                if (recurrenceMatch) {
                  ...
                  if (recurrenceRaw.includes("primeira vez"))
                    finalFields.recurrence_frequency = "primeira_vez";
                  else if (recurrenceRaw.includes("algumas vezes"))
                    finalFields.recurrence_frequency = "algumas_vezes_mes";
                  else if (recurrenceRaw.includes("toda semana"))
                    finalFields.recurrence_frequency = "toda_semana";
                  else if (recurrenceRaw.includes("todos os dias"))
                    finalFields.recurrence_frequency = "todos_os_dias";
                }
        ...
        const timeoutDetected =
          assistantMessage.includes("[TIMEOUT]") ||
