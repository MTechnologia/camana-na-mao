...
export const usePDFExport = () => {
  ...
  const exportToPDF = useCallback(async (options: PDFExportOptions) => {
    ...
    try {
      ...
      pdf.text(`Gerado em: ${formatDateTime(new Date())}`, pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 12;
      ...
      if (options.kpis && options.kpis.length > 0) {
        ...
        options.kpis.slice(0, 4).forEach((kpi, index) => {
          ...
          if (kpi.trend) {
            ...
            pdf.text(`${arrow} ${kpi.trend.value}%`, xPos + kpiWidth / 2, yPosition + 22, { align: 'center' });
          }
        });
      ...
      if (options.statusData) {
        ...
        const total = options.statusData.pending + options.statusData.in_progress + 
                      options.statusData.resolved + options.statusData.rejected;
        const statuses = [
      ...
      if (options.sentimentScore !== undefined) {
        ...
        const color = options.sentimentScore >= 70 ? [34, 197, 94] : 
                      options.sentimentScore >= 40 ? [234, 179, 8] : [239, 68, 68];
      ...
      if (options.filters) {
        ...
        if (options.filters.dateRange?.from || options.filters.dateRange?.to) {
          const from = options.filters.dateRange.from ? formatShortDate(new Date(options.filters.dateRange.from)) : '-';
          const to = options.filters.dateRange.to ? formatShortDate(new Date(options.filters.dateRange.to)) : '-';
        ...
        pdf.text('Câmara Municipal de São Paulo - Câmara na Mão', pageWidth - margin, footerY, { align: 'right' });
      }

      // Save PDF
      const filename = options.filename || `relatorio_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(filename);
