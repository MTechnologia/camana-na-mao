...
export function extractJourneySnapshotFromMetadata(
  metadata: unknown,
): JourneySnapshotMetadata | null {
  ...
  const schema_version =
    typeof snapshot.schema_version === "string" ? snapshot.schema_version : undefined;
  const journey_type =
    typeof snapshot.journey_type === "string" ? snapshot.journey_type : undefined;
  const fields =
...
export function appendMessageByIdIfMissing(
  ...
  const alreadyExists = existing.some((msg) => typeof msg?.id === "string" && msg.id === nextId);
