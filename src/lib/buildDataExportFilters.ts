...
function categoriesFromFilterValue(
  category?: string,
  ...
  if (dataset === "transport_reports") {
    const transport = [...new Set([...slice.transportSubcategories, ...slice.publicServiceTypes])];
...
export function buildDataExportDefaultFilters(
  ...
  const categories = source.categories ?? categoriesFromFilterValue(source.category, dataset);
...
export function dataExportFiltersFromGlobal(
  ...
  return buildDataExportDefaultFilters({ startDate, endDate, category, region }, dataset);
}
  ...
  const filters = buildDataExportDefaultFilters(
    {
      startDate: iso.startDate,
      endDate: iso.endDate,
      categories: input.categoryFilter !== "all" ? [input.categoryFilter] : undefined,
      status: input.statusFilter,
      regions:
        input.regionFilter && input.regionFilter !== "all" ? [input.regionFilter] : undefined,
    },
    defaultDataset,
  );
