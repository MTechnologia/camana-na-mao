...
export async function persistUrbanManagementReferral(
  ...
  let councilReferralId = input.existingCouncilReferralId?.trim() || existingIds[0] || null;
  ...
  const { error: triageErr } = await supabase.from("report_triage").upsert(
    {
      source_table: "urban_reports",
      report_id: input.urbanReportId,
...
export async function loadLatestCouncilReferralByUrbanIds(
  ...
  for (const row of data ?? []) {
    ...
    const commissionName = comm?.name?.trim() || comm?.code?.trim() || null;
    ...
    out.set(reportId, {
      ...
      matchScore:
        typeof row.match_score === "number" ? row.match_score : Number(row.match_score) || 0,
