...
describe("drillThroughMatchers", () => {
  ...
  it("logradouro casa com rótulo do gráfico mesmo sem coluna street", () => {
    ...
    expect(
      matchesStreetBar(bar, "Butantã", "Outra Rua, 10, São Paulo", null, null, "Butantã"),
    ).toBe(false);
  });
  ...
  it("barra Sem logradouro só casa quando o rótulo territorial é fallback", () => {
    ...
    expect(matchesStreetBar(bar, null, null, null, null, DISTRICT_FALLBACK_ID)).toBe(true);
    expect(
      matchesStreetBar(
        bar,
        null,
  ...
  it("filtro de região usa zona derivada do texto", () => {
    ...
    expect(matchesTerritoryBar(bar, "west", "Pinheiros", null, -23.56, -46.69)).toBe(true);
  });
});
