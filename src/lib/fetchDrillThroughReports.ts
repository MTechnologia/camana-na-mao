...
>(query: T, filters: ReportsAnalyticsFilters): T {
  let q = query;
...
function toTransportRow(r: {
  ...
  const label = r.sub_category ? `${r.report_type} · ${r.sub_category}` : r.report_type;
  return {
    id: r.id,
...
function applyTerritorialUrbanQuery<
  T extends {
    or: (filter: string) => T;
    ilike: (col: string, pattern: string) => T;
    eq: (col: string, value: string) => T;
  },
>(query: T, bar: ChartBarPoint, scope?: DrillThroughScope): T {
  let q = query;
...
async function fetchTransportDrillRows(
  ...
  if (bar.filterKey === "category") {
    query = query.or(`report_type.eq.${bar.filterValue},sub_category.eq.${bar.filterValue}`);
...
export function drillThroughTotalPages(
  total: number,
  pageSize: number = DRILL_THROUGH_PAGE_SIZE,
): number {
  if (total <= 0) return 1;
