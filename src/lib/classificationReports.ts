...
function displayId(reportId: string, reportType: string, meta?: ReportMeta): string {
  return (
    meta?.protocolCode ??
    `${REPORT_TYPE_LABELS[reportType] ?? reportType} · ${reportId.slice(0, 8)}`
  );
}
...
function mapPendingRow(
  ...
  return {
    ...
    predictedCategory: row.predicted_category ?? "—",
    confidencePct: null,
    feedbackLabel: row.classification_source ? `Origem: ${row.classification_source}` : undefined,
...
export async function fetchClassificationReports(
  ...
  if (kind === "pending") {
    ...
    const items = rows
      .map((r) => mapPendingRow(r, meta))
      .filter((r): r is ClassificationReportItem => !!r);
  ...
  const items = rows
    .map((r) => mapVsRow(r, meta))
    .filter((r): r is ClassificationReportItem => !!r);

  const total =
    kind === "precision" ? (rows.length > 0 ? totalCount : 0) : totalCount || items.length;
