-- Faixas em anel (ex.: 2,1–5 km): o REST com LIMIT sem ORDER BY distância excluía pontos da faixa.
-- Parâmetro opcional min_radius_meters filtra no servidor após Haversine, com ORDER BY dist ASC + LIMIT.
...
COMMENT ON FUNCTION public.search_public_services_fulltext IS
  'Lista public_services no bbox + raio Haversine; min_radius_meters opcional (anel); FTS/ILIKE; timeout 90s.';
