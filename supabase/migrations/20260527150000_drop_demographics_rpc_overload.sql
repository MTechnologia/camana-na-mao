-- Remove overload antiga de get_reports_with_demographics (9 params) que conflita com a versão
-- com p_status (10 params) e causa PGRST203 no PostgREST.

DROP FUNCTION IF EXISTS public.get_reports_with_demographics(
...
-- Garante assinatura canônica única (reaplica corpo da migration de status).
  ...
  WITH filtered_urban AS (
    ...
    WHERE (p_report_type IS NULL OR p_report_type = 'urban')
      ...
      AND (
        ...
        OR ur.category = ANY (p_categories)
      )
  ...
  filtered_transport AS (
    ...
    WHERE (p_report_type IS NULL OR p_report_type = 'transport')
      ...
      AND (
        p_categories IS NULL
        OR cardinality(p_categories) = 0
        OR tr.report_type = ANY (p_categories)
        OR tr.sub_category = ANY (p_categories)
      )
  ...
  filtered_evaluations AS (
    ...
    WHERE sr.publication_status = 'published'
      ...
      AND (
        p_categories IS NULL
        OR cardinality(p_categories) = 0
        OR ps.service_type::text = ANY (p_categories)
      )
...
REVOKE ALL ON FUNCTION public.get_reports_with_demographics(
  text, text, text, text, text, timestamptz, timestamptz, text[], text
) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_reports_with_demographics(
  text, text, text, text, text, timestamptz, timestamptz, text[], text
) TO authenticated;
